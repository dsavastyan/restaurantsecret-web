...
export default function AccountSubscription() {
  const { token, reload, sub } = useOutletContext<AccountOutletContext>();
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      if (sub?.status) {
        setStatusData({ status: sub.status, expires_at: sub.expires_at || null });
        setError(null);
      } else {
  ...
  }, [accessToken, logout, sub?.status, sub?.expires_at]);
